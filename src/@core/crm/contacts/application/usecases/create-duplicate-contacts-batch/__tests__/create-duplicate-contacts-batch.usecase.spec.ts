...
import { CreateDuplicateContactsBatchUseCase } from '@/@core/crm/contacts/application/usecases/create-duplicate-contacts-batch/create-duplicate-contacts-batch.usecase'
...
describe('CreateDuplicateContactsBatchUsecase unit tests', () => {
  it('Deve executar sem erros', async () => {
    const contactRepositoryMock = {
      createBatch: jest.fn(),
    }

    const useCase = new CreateDuplicateContactsBatchUseCase(
  ...
  it('Deve criar as associações corretamente', async () => {
    ...
    await useCase.execute(contacts)
