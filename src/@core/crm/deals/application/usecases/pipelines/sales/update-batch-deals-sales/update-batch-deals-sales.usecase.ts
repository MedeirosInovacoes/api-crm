...
import { UpdateBatchDealsLeadRetrievalOutputDto } from '@/@core/crm/deals/application/usecases/pipelines/lead-retrieval/update-batch-deals-lead-retrieval/update-batch-deals-lead-retrieval.dto'
...
export class UpdateBatchDealsSalesUseCase {
  constructor(private readonly dealsRepository: DealGateway) {}

  execute = async (
    input: UpdateBatchDealsSalesInputDto,
    ...
    for (const [index, object] of input.entries()) {
      ...
      const entity = CreateDealWithStageHelper.execute({
        ...
        newStage: env.NEWS_CUSTOMERS_PIPELINE_DONE_DEAL_ID,
      })
